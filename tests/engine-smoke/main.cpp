...
std::string templated_prompt(const SynapEngine* engine,
    ...
    int32_t n = synap_engine_apply_chat_template(
        engine, roles.data(), contents.data(),
        static_cast<int32_t>(msgs.size()), true, buf.data(),
...
void print_stats(const SynapEngine* engine, const char* label) {
    ...
    printf("\n[%s] prompt=%d reused=%d prefilled=%d (%.0f ms) | decoded=%d (%.0f ms, %.1f tok/s)\n",
           label, st.prompt_tokens, st.prefill_reused, st.prefill_new,
           st.prefill_ms, st.decode_tokens, st.decode_ms, decode_tps);
}
...
int main(int argc, char** argv) {
    ...
    printf("model: %s\nvision=%d audio=%d n_ctx=%d\n",
           desc, synap_engine_has_vision(engine), synap_engine_has_audio(engine),
           synap_engine_n_ctx(engine));
    ...
    if (p1.empty()) { fprintf(stderr, "FAIL: chat template\n"); synap_engine_free(engine); return 3; }
