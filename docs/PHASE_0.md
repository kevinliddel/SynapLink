...
| Piece | Location |
|---|---|
| llama.cpp pin (b9596, Gemma 4 mtmd vision+audio upstream) | cloned by `scripts/build-llama-xcframework.sh` into `third_party/llama.cpp` (gitignored) |
...
- Model download/checksum UI is Phase 1 (`ModelManager`); Phase 0 side-loads
  via file sharing.
- llama.cpp upgrades: bump `LLAMA_TAG` in `scripts/build-llama-xcframework.sh`,
  rebuild, and re-run `scripts/test-engine-macos.sh`.
